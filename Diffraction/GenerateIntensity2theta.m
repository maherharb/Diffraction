...
% Example: 
% load 'graphite.mat';
% Probe.Energy=8048.3; % Cu K alpha 1 radiation
% Probe.Type='xray';
% Generate_Intensity_2theta(Lattice, Probe)
%
% Last updated 5-17-2017 Cosmin Popescu

function [Table]=GenerateIntensity2theta(Lattice, Probe,FigNum,hkl,Threshold, Separation)
%% Get the XRD Pattern
...
if nargin>2
    ...
    xlabel('2\theta [deg]');
    ylabel('Intensity');
    Title=strcat('Scatter plot of individual reflections for (',Lattice.Symbol,'-',Lattice.Type,')');
    title(Title);
    axis([0 180 0 1.05*max(MainData(:,4))]);
    hold on;
...
xseparation=-1.5;
yseparation=max(MainData(:,4))*0.03;
    ...
    if (angle<MainData(countAngle,5))&& (angle+Separation>MainData(countAngle,5)&&countAngle<length)
        ...
        if countAngle<length
            if MainData(countAngle+1,5)-MainData(countAngle,5)<Separation
                warning('The separation between some peaks is less than the Separation. Please decrease the value for Separation.');
...
hold on;
plot(XRD_plot(:,1),XRD_plot(:,2)/max(XRD_plot(:,2)),'-b','linewidth',1.5);
xlabel('2\theta [deg]');
ylabel('Relative Intensity');
Title=strcat('Powder diffraction of (',Lattice.Symbol,'-',Lattice.Type,') at (',num2str(Probe.Energy),' eV) (',Probe.Type,')');
title(Title);
axis([0 180 0 1.05]);
hold on;
box on;
